...
This section gives a very brief overview of AES and, most importantly, emphasizes specific synthesis challenges presented during each stage of the encryption process. Additionally, this section will briefly explain what it means to operate AES in Counter Mode.

AES is a symmetric key block cipher encryption algorithm that operates on 128-bit blocks of data using keys of length 128, 192 or 256 bits; however, the authors chose to only consider implementations of the algorithm using 128-bit keys. The algorithm uses four main transformation functions to provide cryptographic diffusion: SubBytes, ShiftRows, MixColumns and AddRoundKey. ShiftRows is the least computationally intensive of the transformations and simply involves rotating bytes in an array. SubBytes requires significantly more computation to determine modular inversions in a finite field as well as perform an affine transformation; although, since SubBytes is performed on individual bytes, the calculations can be carried out by pre-computing all possible values and then referencing a $2^8=256byte$ lookup table. AddRoundKey uses the same lookup table, also called a Rijndael S-box \cite{16}, from the SubBytes transformation
